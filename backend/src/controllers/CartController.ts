...
export class CartController extends Controller {
    ...
    public async getCartItems(@Path() userId: string): Promise<ICartItem[] | null> {
    ...
    public async addItem(@Path() userId: string, @Body() requestBody: CartItemUpdate): Promise<ICartItem> {
        this.setStatus(201);
    ...
    public async updateItemQuantity(@Path() itemId: string, @Path() quantity: number): Promise<ICartItem> {
