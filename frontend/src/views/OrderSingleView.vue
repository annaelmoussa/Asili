      ...
      <Card class="mb-6">
        <CardHeader class="flex flex-col md:flex-row justify-between items-center">
          ...
          <div class="text-right">
            ...
            <p>Statut : <span :class="statusClass(order.status)">{{ order.status }}</span></p>
          </div>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div v-for="item in order.items" :key="item._id" class="flex items-start mb-4">
            <img :src="item.productImage" alt="Product Image" class="w-24 h-24 object-cover mr-4">
            <div>
              <p class="font-semibold">{{ item.productName }}</p>
              <p>{{ item.quantity }}x - {{ formattedPrice(item.priceAtPurchase) }}</p>
              <Button variant="link" class="mt-2 text-blue-500" @click="handleReorder(item.productId)">Acheter à nouveau</Button>
            </div>
          </div>
        </CardContent>
      </Card>
